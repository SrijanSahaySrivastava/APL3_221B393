  ...
  for (let fellow of fellows) {
    try {
      const res = await axios.post(
        `http://localhost:${fellow}/start-election`,
        {
          term: currentTerm,
          candidateId: port,
        }
      ...
      if (res.data.voteGranted) {
        voteReceived++;
        if (voteReceived > (fellows.length + 1) / 2) becomeLeader();
      }
    } catch (err) {
      console.log(`Failed to contact fellow ${fellow}`);
      //console.log(err);
    }
  }
}
function becomeLeader() {
  ...
  if (electionTimeout) clearTimeout(electionTimeout); //stop contesting, you’ve become leader
  if (heartBeatInterval) clearInterval(heartBeatInterval);

  heartBeatInterval = setInterval(() => {
    for (const fellow of fellows) {
      axios
        .post(`http://localhost:${fellow}/heartbeat`, {
          leader: port,
          term: currentTerm,
        })
        .catch((err) =>
